...
describe('Storage Adapters', () => {
  ...
  describe('cookies Adapter', () => {
    ...
    it('setItem creates cookie with data', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith(expect.stringContaining('test-key='))
    })
    ...
    it('removeItem sets cookie with past expiration', async () => {
      ...
      expect(setSpy).toHaveBeenCalledWith(expect.stringContaining('Max-Age=0'))
    })
  })
  ...
  describe('Adapter Integration', () => {
    ...
    it('adapters handle empty string values', async () => {
      const adapters = [localStorageAdapter(), sessionStorageAdapter(), cookiesAdapter()]
